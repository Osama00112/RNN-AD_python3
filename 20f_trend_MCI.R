...
data <- read_csv("20f_MCI_trend.csv")
...
data_long <- data_long %>%
  mutate(Trend = case_when(
    `Feature Name` == "MidTemp" ~ "rising",
    `Feature Name` == "WholeBrain" ~ "rising",
    TRUE ~ "falling"
  )) %>%
  mutate(Color = case_when(
    Trend == "rising" & `Feature Name` == "MidTemp" ~ "red",  # lighter red
    Trend == "rising" & `Feature Name` == "WholeBrain" ~ "darkorange",  # medium red
    Trend == "falling" & `Feature Name` == "DX: CN" ~ "darkgreen",  # dark green
    Trend == "falling" & `Feature Name` == "FAQ" ~ "green",  # green
    Trend == "falling" & `Feature Name` == "ICV" ~ "lightgreen",  # light green
    Trend == "falling" & `Feature Name` == "FDG" ~ "lightgreen"
  ))

# Define the order of the legend items
legend_order <- c("red", "darkorange", "darkgreen", "green", "lightgreen")
...
ggplot(data_long, aes(x = Month, y = Attribution, group = `Feature Name`, color = Color)) +
  ...
  scale_color_manual(
    values = c("red" = "red", "darkorange" = "darkorange",  
               "darkgreen" = "darkgreen", "green" = "green", 
               "lightgreen" = "lightgreen"),
    breaks = legend_order,
    labels = c("MidTemp", "WholeBrain","DX: CN", "FAQ", "FDG, ICV")
  ) +
